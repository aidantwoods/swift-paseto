...
public extension Token {
    func sign<V>(with key: AsymmetricSecretKey<V>) throws -> Blob<Signed<V>, V> {
    ...
    func encrypt<V>(with key: SymmetricKey<V>) throws -> Blob<Encrypted<V>, V> {
        guard let claimsData = serialisedClaims else {
            throw Exception.serialiseError(
                "The claims could not be serialised."
            )
...
public extension Token {
    func sign<V>(with key: AsymmetricSecretKey<V>) -> Blob<Signed<V>, V>? {
        return try? sign(with: key)
    }
    func encrypt<V>(with key: SymmetricKey<V>) -> Blob<Encrypted<V>, V>? {
